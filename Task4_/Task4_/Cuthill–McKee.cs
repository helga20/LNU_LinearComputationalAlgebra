...
namespace Task4_
{
    public class CuthillMcKee
    {
        ...
        public void Cuthill_McKee_Algorithm(List<List<int>> g_matrix)
        {
            ...
            MakeNewGraph(g_graph, renumbering);
        ...
        public  void MakeNewGraph(List<List<int>> old, List<int> perenum)
        {
            bool[,] graph = new bool[old.Count, old.Count];
            ...
            for (int i = 0; i < old.Count; i++)
            {
                for (int j = 0; j < old.Count; j++)
                {
                    Console.Write(graph[i, j] == true ? "1" : "-");
                    Console.Write(" ");
                }
                Console.WriteLine();
